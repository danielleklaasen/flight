<nav class="main">
    <div class="container">
        <div class="menu-burger btn-menu">
            <img src="http://f.cl.ly/items/1U2c3b1215383h3a2T2r/icon-menu.svg"/>
        </div>
        <div class="logo-container">
            <img class="logo btn-home" src="assets/logo.svg" alt />
            <img class="airplane btn-home" src="assets/airplane.svg" alt />
        </div>
        <div class="profile circle">
            <img class="profile-icon btn-wdw-profile" src="assets/icons/outline/profile.svg" alt/>
        </div>
    </div>

</nav>

<div class="menu">
    <div class="btn-close-menu"></div>
    <a id="wdw-template" href="javascript:void(0)" class="btn-wdw menu-item link-disappear-primary">Template</a>
    <a id="wdw-flight" href="javascript:void(0)" class="btn-wdw menu-item link-disappear-primary">Home</a>
    <a id="wdw-results" href="javascript:void(0)" class="btn-wdw menu-item link-disappear-primary">Results</a>
    <a id="wdw-profile" href="javascript:void(0)" class="btn-wdw menu-item link-disappear-primary">Profile</a>
    <div class="button-container">
        <button id="wdw-login"class="btn-wdw menu-item btn-2">Login</button>
        <button id="wdw-register" class="btn-wdw menu-item sub-item btn btn-2">Register</button>
    </div>

</div>
